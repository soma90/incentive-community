...
module.exports = {
  ...
  user_transfer_post: async (req, res) => {
    //로그인 체크
    if (!req.session.userData || req.session.userData.id !== req.body.id) {
    ...
    //정보 업데이트
    const userResult = await comContr.user_amountUpdate(
      req.session.userData.id,
      req.session.userData.address
    );
    const toResult = await comContr.user_amountUpdate(
      req.body.toId,
      req.body.toAddr
    );
  ...
  user_faucet_post: async (req, res) => {
    ...
    //이더전송
    const sendResult = await common.sendEther(
      req.session.userData.address,
      "1"
    );
    if (!sendResult) {
    ...
    //정보 업데이트
    const updateResult = await comContr.user_amountUpdate(
      req.session.userData.id,
      req.session.userData.address
    );
    if (updateResult) {
      return res.status(200).send({
        message: "success",
        data: { eth_amount: updateResult.eth_amount },
      });
    }
    res.status(400).send({ message: "error" });
  },
  user_joinById_get: async (req, res) => {
    ...
    if (result) {
      ...
      if (result.length > 0) {
        res
          .status(409)
          .send({ message: "already exists", data: { isDuplicate: true } });
        return;
      }
      //중복된 아이디가 없을때
      else {
  ...
  user_join_post: async (req, res) => {
    ...
    try {
      await sequelize.transaction(async (tx) => {
        //지갑생성
        let address = "";
        address = await common.newAccount(req.body.password);
        if (!address) {
        ...
        //블록체인저장
        const dsResult = await dataStore.setUserData(
          result.id,
          result.nickname,
          result.address
        );
  ...
  user_login_post: async (req, res) => {
    const result = await user.user_login_post(
      req.body.nickname,
      req.body.password
    );
    if (result) {
